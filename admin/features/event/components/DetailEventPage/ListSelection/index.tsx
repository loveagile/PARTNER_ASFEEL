...
import ListSelectionComponent from './ListSelection'
...
const ListSelectionFeature = ({ detailEvent }: ListSelectionFeatureProps) => {
  ...
  const modifySelectionList = async (data: UserData[]) => {
    try {
      const res = await customFetchUtils(API_ROUTES.EVENT.modifySelection, {
  ...
  const handleFetchData = async (searchParams?: { [key: string]: any }) => {
    ...
    try {
      const res = await customFetchUtils(
        `${API_ROUTES.EVENT.getSelection}?${urlSearchParams || ''}`,
        {
