...
type FilterProps = {
  ...
  eventTypes: CheckboxOptionType[]
  eventNames: CheckboxOptionType[]
...
const Filter = ({
  ...
  const title = React.useMemo(() => {
    if (recruitmentStatus === COMMON_STATUS.IN_PREPARATION) {
      return '準備中'
    }
    ...
    if (recruitmentStatus === COMMON_STATUS.FINISH) {
      return '終了'
    }
  }, [recruitmentStatus])
    ...
    <Form form={form} className="mb-6" onValuesChange={handleValuesChange}>
      <Row justify="space-between" align="middle" className="mb-2 gap-2">
        <Col>
          <Row align="middle">
            ...
            {recruitmentStatus === COMMON_STATUS.IN_PUBLIC && (
              <Col>
                <Form.Item
                  name="isOnly"
      ...
      {isShowFilter && (
        <div className="rounded-xl bg-gray-gray_light px-8 py-3 text-small">
          ...
          <Form.Item label="部活タイプ" className="mb-2" name="eventType">
            <Select
              allowClear
              showSearch
              filterOption={filterOption}
              placeholder="部活タイプを選択"
              options={eventTypes}
              className="select_filter"
              popupClassName="select_filter_popup"
              suffixIcon={<AiFillCaretDown className="text-gray-black" />}
          ...
          <Form.Item label="種目" className="mb-2" name="eventName">
            <Select
              showSearch
              filterOption={filterOption}
              allowClear
