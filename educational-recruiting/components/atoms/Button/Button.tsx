...
const Button: React.FC<ButtonProps> = ({
  ...
  return (
    <button
      className={
        // w-[150px]
        'flex flex-row items-center relative whitespace-nowrap pc:text-base sp:text-sm ' + backgroundColor + shapeStyle + opacity + className
      }
      onClick={onclick}
      disabled={disabled}
      style={style}
    >
      {arrow == ButtonArrow.LEFT && <FontAwesomeIcon icon={faChevronLeft} className={'absolute pc:w-4 pc:h-4 sp:w-3 sp:h-3 ' + arrowPaddingStyle} />}
      <div className={'flex items-center m-auto ' + textClassName}>
        {icon == ButtonIcon.FRONT && iconComponent}
        {text}
        {icon == ButtonIcon.BACK && iconComponent}
      </div>
      {arrow == ButtonArrow.RIGHT && (
        <FontAwesomeIcon icon={faChevronRight} className={'absolute pc:w-4 pc:h-4 sp:w-3 sp:h-3 ' + arrowPaddingStyle} />
      )}
    </button>
  )
}
