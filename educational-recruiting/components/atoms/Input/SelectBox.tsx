...
const ArrowIcon = () => {
  return (
    <svg
      ...
      className="pc:w-[10px] pc:h-[6px] sp:w-[8px] sp:h-[4px] absolute pc:right-[15.5px] sp:right-[15px] top-1/2 transform -translate-y-1/2 "
    >
...
const SelectBox = ({ value, status, options, size, setValue, className = ' w-[200px]' }: SelectBoxProps) => {
  ...
  return (
    <div className={'relative '} ref={selectRef}>
      <div
        className={
          'appearance-none border rounded m-auto hover:cursor-pointer flex flex-row items-center ' +
          selectBoxStyle +
      ...
      >
        <ArrowIcon />
        {selectedIcon == IconTypeSelectBox.FRONT && selected?.iconComponent && selected?.iconComponent(isPlaceholder, selectedIcon, size)}
        {selected?.text?.length > 25 ? selected?.text.substring(0, 25) + '...' : selected?.text}
        {selectedIcon == IconTypeSelectBox.BACK && selected?.iconComponent && selected?.iconComponent(isPlaceholder, selectedIcon, size)}
      </div>
      <ul className={`bg-white overflow-y-auto absolute z-10 shadow-xl ${open ? 'max-h-60' : 'max-h-0'} ` + className}>
        <div className="sticky top-0 flex items-center px-2 bg-white"></div>
        {options.map((option, index) => (
          <li
            key={option?.value}
            className={`p-2 text-sm flex flex-row pl-4
          ...
          >
            {option.icon == IconTypeSelectBox.FRONT && option.iconComponent && option.iconComponent(option.placeholder || false, option.icon, size)}
            {option?.text}
            {option.icon == IconTypeSelectBox.BACK && option.iconComponent && option.iconComponent(option.placeholder || false, option.icon, size)}
          </li>
        ))}
      </ul>
    </div>
