...
const TabItem = ({ notice, url, text, size, setActiveItem, activeItem, index }: TabItemProps) => {
  const colorStyle = activeItem == index ? 'text-core-blue_dark border-b-4 border-core-blue_dark ' : 'text-gray-black opacity-40 '
  let textStyle = ''
  switch (size) {
    case TabSize.PC:
  ...
  return (
    <Link href={url} className={'items-center py-[10px]  ' + colorStyle + textStyle} onClick={() => setActiveItem(index)}>
      {text}
      {notice == State.ON && (
        <>
          <FontAwesomeIcon className={'w-[10px] mb-3 text-core-red absolute  ' + noticeStyle} icon={faCircle} />
          <div className="w-[10px] mb-3 hidden" />
        </>
      )}
    </Link>
    // <div
