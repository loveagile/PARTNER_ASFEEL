...
export const Sidebar = ({ groups, setActiveItem, activeItem, setCurrentPage }: SidebarProps) => {
  ...
  return (
    <aside className="w-[180px] bg-gray-white gap-1 flex flex-col pb-2 drop-shadow-drop_wide">
      {groups.map((group, indexGroup) => {
        return (
            ...
            {group.items.map((item, indexItem) => {
              return (
                <div key={indexGroup + '-' + indexItem}>
                    ...
                    <div key={indexGroup + '-' + indexItem + '-select-box'} className="relative flex w-max">
                      ...
                      >
                        ...
                        >
                          ...
                          <MdArrowRight size={24} className="absolute top-0 bottom-0 my-auto right-[13.5px] text-gray-gray_dark" />
                        </div>
                      </div>
                      {showOptions && selectedSelect.group == indexGroup && selectedSelect.item == indexItem && (
                        <div ref={optionsRef} className="w-[200px] absolute left-[100.5%] top-[50%]">
                          {item.item.options.map((option, indexOption) => (
                            <div
                              className={
                  ...
                  {!instanceOfSelectBoxProps(item.item) && !instanceOfSelectProps(item.item) && instanceOfButtonType(item.item) && (
                    <div className="py-[6px]" key={indexGroup + '-' + indexItem + '-button'}>
                      ...
                      >
                        ...
                        <div className="pr-[10px] py-[6px]">{item.item.text}</div>
                      </button>
                    </div>
                  )}
                </div>
