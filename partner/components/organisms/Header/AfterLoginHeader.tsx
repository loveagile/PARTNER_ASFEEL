...
export const AfterLoginHeader = () => {
  ...
  var [profileInfo, setProfileInfo] = useState<PrivateUser>(userInfo)
  ...
  useEffect(() => {
    ...
    if (token) {
      const parseToken: PrivateUser = JSON.parse(decrypt(token))
      setProfileInfo(parseToken)
    }
  }, [])
  ...
  return view == 'PC' ? (
    <div className="fixed z-40 flex h-[64px] w-full flex-row items-center justify-between bg-core-blue_dark p-[10px]">
      <HeaderLogo
        onClick={() => {
      ...
      <div className="flex items-end space-x-2">
        ...
        <div className="flex space-x-2">
          <p className="ml-3 flex items-end font-bold text-white">{`${profileInfo.name.sei}${profileInfo.name.mei}`}</p>
          <div
          ...
          >
            {profileInfo.avatar ? <Icon src={profileInfo.avatar} alt="profile picture" size={25} /> : <ProfileIcon />}
          </div>
        </div>
      </div>
    </div>
  ) : (
    <div className="fixed z-20 flex h-[64px] w-full flex-row items-center justify-between bg-core-blue_dark p-[10px]">
      <div className="flex items-center text-[12px] font-bold text-gray-white hover:cursor-pointer">
        <div className="inset-y-0 left-0 flex items-center ">
          ...
          >
            <div className="">
              <FontAwesomeIcon icon={faBars} className="h-[2em]" />
              {notiCount > 0 && (
                <FontAwesomeIcon className={'absolute left-8 mb-3 w-[8px] text-core-red'} icon={faCircle} />
              )}
            </div>
          </button>
        </div>
      ...
      {isNavbarOpen && (
        <div className="absolute left-0 top-0 z-20 mt-[64px] flex w-[200px] flex-col gap-4 rounded-lg bg-white py-4 text-timestamp text-core-blue_dark">
          <div
            onClick={() => {
              dispatch(setStoreIsNavbarOpen(false))
              router.push('/notifications')
