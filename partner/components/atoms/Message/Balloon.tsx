...
const Balloon: React.FC<BalloonProps> = ({ className = '', textContent, balloonType, balloonColor }) => {
  ...
  return (
    <div
      className={`relative ${balloonType === BalloonType.Sent && 'pl-2'} ${
        balloonType === BalloonType.Received && 'pr-2'
      }`}
    >
      ...
      <div className={`${className} ${styleClass} inline-block rounded-[10px] p-[10px]`}>
        <pre className="whitespace-break-spaces break-all align-baseline text-body_sp">{textContent}</pre>
      </div>
    </div>
  )
}
