    ...
    <div className={`${className}`}>
      <table className="w-full text-center">
        ...
        <tbody>
          {amPm.map((time) => (
            <tr key={time}>
              <td className={`${textClass} border border-gray-gray bg-gray-gray_lighter ${cellSize} py-[5px]`}>
                {convertToJapaneseAmPm(time)}
              </td>
              {daysOfWeek.map((day) => (
                <td
                  key={`${day}-${time}`}
                  className={`border border-gray-gray align-middle ${
                    size === 'small' ? 'bg-gray-white' : ''
                  } ${cellSize}`}
                >
                  {hasSchedule(day, time) ? (
                    <span className={`mx-auto block ${blackDotSize} rounded-full bg-gray-black`}></span>
                  ) : null}
                </td>
              ))}
            </tr>
