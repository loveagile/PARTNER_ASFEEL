...
import { projectCandidateAtom, projectCandidatePaginationAtom } from "@/recoil/atom/projectCandidateAtom";
...
export const Top = ({ caption, project, statusTabIndex }: ThisProps) => {
  ...
  const [_, setCandidateList] = useRecoilState(projectCandidateAtom);
  const paginationData = useRecoilValue(projectCandidatePaginationAtom);
  ...
  useEffect(() => {
    ...
    const fetchProject = async ({
      ...
      const result = await axios.post(`/api/projects/candidate/search?organization=${keyword}&isOnly=${isOnly}&isShowNG=${isShowNG}&desiredGender=${desiredGender}&desiredAge=${desiredAge}&desiredQualifications=${desiredQualifications}&other=${other}&projectId=${project.id}`);
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div>
          <div className="flex flex-wrap items-end gap-3 pc:gap-5 p-[10px] text-small mt-5">
            ...
            <span>候補選出条件</span>
            <span>種目 : {project.eventName}</span>
            <span>指導可能地域 : {project.workplace.prefecture} {project.workplace.city} {project.workplace.address1}</span>
            <button
              type="button"
