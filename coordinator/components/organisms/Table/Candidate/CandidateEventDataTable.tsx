...
import CandidateEventDataTablePagination from './CandidateEventDataTablePagination'
...
import { eventCandidateAtom } from '@/recoil/atom/events/eventCandidateAtom'

export interface CandidateEventItem {
  docId: string
  projectId: string
  userId: string
...
const CandidateEventDataTable = ({
  event,
  isCommitteeAccount,
  onClick,
  statusTabIndex,
}: ThisProps) => {
  const candidates = useRecoilValue(eventCandidateAtom)
  const [tableItems, setTableItems] = useState<CandidateEventItem[]>([])
  ...
  const handleChange = (item: CandidateEventItem, checked: boolean) => {
    if (checked) {
  ...
  const selectedCandidates: CandidateEventItem[] = tableItems.filter((item) =>
  ...
  const onNgClick = async () => {
    ...
    await selectedCandidates.map((candidate) => {
      ...
      async function updateCandidate() {
        let docRef = DocRef.eventProjectsScoutList(
          candidate.projectId,
          candidate.userId,
        )
    ...
    const statusResult = await axios.post(
      `/api/events/candidate/search?projectId=${event.id}&status=unsend`,
    )
  ...
  return (
    <div>
      ...
      <CandidateEventDataTablePagination
...
export default CandidateEventDataTable
