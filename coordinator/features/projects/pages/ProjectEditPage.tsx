...
import { useProjectEditProvider } from '../providers/useProjectEditProvider';
...
function ProjectEditPage() {
  const {
    ...
    closeCancelModal,
    isLoading
  } = useProjectEditProvider();

  const { control, handleSubmit, trigger, setValue, formState: { dirtyFields } } = methods;
  let clubTypeLargeCategoryOptions: ISelectOptionsProps[] = clubTypeLargeCategories.map(item => {
    return { value: item.name, label: item.name }
  })
  clubTypeLargeCategoryOptions.unshift({ value: '', label: '選択してください', isDisabled: true })
    ...
    <>
      ...
      {(selectedTypeOption) ?
        <div className="relative flex flex-col w-full p-5 pb-0 text-center pc:p-10 pc:pb-0 bg-gray-gray_lighter">
          ...
          <h1 className="text-h2 pc:text-h1 mb-8 pc:mb-10">募集依頼内容編集</h1>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-8 pc:gap-10">
              <div className="flex flex-col pb-5 pc:pb-10 gap-5 pc:gap-10 pc:max-w-[880px] w-full text-left mx-auto bg-gray-white rounded-[10px] border border-gray-gray">
                ...
                <div className="flex flex-col gap-3 px-5 pc:gap-4 pc:px-10">
                  ...
                  {selectedTypeOption === '学 校' && (
                    <>
                      <Input
                        ...
                        attention={<>当てはまる学校が無い場合は<Link href="https://www.notion.so/844496564e0740e9b293f190f687836f?pvs=4" target='_blank' className="underline text-core-blue">こちら</Link></>}
                      />
                      <Modal isOpen={isSchoolSearchModalOpen} onClose={closeSchoolSearchModal}>
                        ...
                        <div className="mt-5 pc:mt-[30px] min-w-[450px]">
                          <label htmlFor="" className="text-h5 pc:text-h4">学校区分</label>
                          <SelectBox
                            control={control}
                                ...
                                <div className="relative mt-1">
                                  <div className="relative w-full overflow-hidden text-left cursor-default bg-gray-white">
                                    <Combobox.Input
                                      ...
                                      // displayValue={(school: any) => school.name}
                                      placeholder="検索"
                                    ...
                                    {filteredSchools.length === 0 && query !== '' ? (
                                      ...
                                      filteredSchools.map((school) => (
                                        <Combobox.Option
                                          ...
                                          value={school}
                                          onClick={() => {
                                            setValue('organizationName', school.name);
                  ...
                  {selectedTypeOption === '合同チーム' && (
                    <>
                      <div className="flex flex-col gap-1 pc:gap-2">
                        <LabelSub htmlFor="target" text="対象" />
                        <CheckBoxGroup
                          className="w-[184px]"
                          name="target"
                ...
                <div className="flex flex-col gap-3 px-5 pc:gap-4 pc:px-10">
                  ...
                  <div className="flex flex-col gap-1 pc:gap-2">
                    ...
                    <SelectBox
                      ...
                      attention={<>当てはまる種目が無い場合は<Link href="https://www.notion.so/06ab7f25ea5c4c288a2f9fa8f21dd798?pvs=4" target='_blank' className="underline text-core-blue">こちら</Link></>}
                    />
                  </div>
                  <div className="flex flex-col gap-1 pc:gap-2">
                ...
                <div className="flex flex-col gap-3 px-5 pc:gap-4 pc:px-10">
                  ...
                  <div className="flex flex-col gap-4 pc:pl-5">
                    {days.map((day, key) => (
                      <div key={key} className="flex flex-wrap items-center gap-5 pc:gap-10">
                        <label htmlFor={`workingHours_${day.toLowerCase()}`} className="text-h5 pc:text-h4">{convertToJapaneseDayName(day.toLowerCase())}</label>
                        <CheckBoxGroup
                    ...
                    <div className="flex flex-wrap gap-5 pc:gap-6">
                      <label htmlFor="workingHours_note" className="text-h5 pc:text-h4">補足</label>
                      <TextArea
                        control={control}
                        className="flex-grow"
                ...
                <div className="flex flex-col gap-3 px-5 pc:gap-4 pc:px-10">
                  <Label htmlfor="activityDescription" text="活動の紹介" required />
                  <TextArea
                    control={control}
                    className="w-full"
                    name="activityDescription"
                    trigger={trigger}
                    attention={<>例1）県大会ベスト4以上の実績があり全国大会を目指しています<br />例2）初心者中心で部員数も少ないですが楽しみながら頑張っています</>}
                  />
                </div>
              </div>
              <div className="flex flex-col pb-5 pc:pb-10 gap-5 pc:gap-10 pc:max-w-[880px] w-full text-left mx-auto bg-gray-white rounded-[10px] border border-gray-gray">
                ...
                <div className="flex flex-col gap-3 px-5 pc:gap-4 pc:px-10">
                  <Label htmlfor="desiredQualifications" text="資格に関する希望" />
                  <TextArea
                    control={control}
                    className="w-full"
                    name="desiredQualifications"
                    trigger={trigger}
                    attention={<>例1）部活動指導員や教員免許を取得もしくは取得予定<br />例2）競技指導資格の取得を将来的に考えている方</>}
                  />
                </div>
                <div className="flex flex-col gap-3 px-5 pc:gap-4 pc:px-10">
